...
export default function CaravanDetailsPage({ caravan, onCompare, onProceedBooking, onGoHome, onLogout, onAddCaravan, onBack }) {
  ...
  const galleryImages = caravan?.details?.displayImages?.length
    ...
    : [truckImage];
  const mainImage = galleryImages[0] || truckImage;
    ...
    <Box sx={{ bgcolor: '#fff', minHeight: '100vh', pb: 0 }}>
      <Container maxWidth="lg">
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                {galleryImages.slice(0, 6).map((image, i) => (
                  <Box key={`${image}-${i}`} sx={{ width: 60, height: 45, borderRadius: 1, overflow: 'hidden', cursor: 'pointer', border: i===0 ? '2px solid #52d88d' : 'none' }}>
                    <img src={image || truckImage} alt={`Thumbnail ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </Box>
                ))}
              </Box>
              <Box sx={{ flex: 1, borderRadius: 2, overflow: 'hidden', height: 330 }}>
                <img src={mainImage} alt="Caravan" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </Box>
            </Box>
          </Grid>
        ...
        <Box sx={{ mb: 8 }}>
          <Box sx={{ display: 'flex', gap: 6, borderBottom: '1px solid #e5e7eb', mb: 4 }}>
            {['Bedroom', 'Restroom', 'Kitchen', 'Living room', 'Others'].map((tab, i) => (
              <Typography key={tab} sx={{ fontWeight: 700, pb: 1.5, cursor: 'pointer', borderBottom: i===0 ? '3px solid #52d88d' : 'none', color: i===0 ? '#111827' : '#6b7280' }}>{tab}</Typography>
            ))}
          </Box>
          
          <Grid container spacing={3}>
            {[
              { label: "Sleeping Bed (In Built)", img: truckImage },
              { label: "Sleeping Bed Pop Up", img: truckImage },
              { label: "Sleeping Sofa", img: null },
              { label: "Mobile Charging point", img: truckImage },
              { label: "Moon roof/ sun roof", img: truckImage },
              { label: "Fan", img: null },
              { label: "Air Conditioner", img: null },
            ].map((item, i) => (
      ...
      <Box sx={{ bgcolor: '#52d88d', color: '#fff', py: 8, px: { xs: 4, md: 10 } }}>
        <Grid container spacing={8}>
          <Grid size={{ xs: 12, md: 5 }}>
            <Box sx={{ mb: 3, bgcolor: '#fff', width: 'fit-content', p: 1, borderRadius: 1 }}><img src={footerLogo} alt="TravelKeet Logo" style={{ height: 35 }} /></Box>
